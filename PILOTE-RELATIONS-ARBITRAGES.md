...
- **B — card de collection** : card d'article naviguant vers son détail, deux actions internes (partager, archiver). Piège : navigation vs action, calme de la surface vs visibilité des actions internes.
...
**Qualité de l'interface** — (a) checklist déterministe et semi-déterministe (pass/fail) : les MESURE de la tranche — action jamais présentée en lien ; navigation en `<a href>`, action en `button` ; contraste 3:1 ; focus visible ; cible tactile ; surface statique sans gestionnaire de clic ; au plus un rang dominant par vue (semi-déterministe, convention de harnais, § 8) ; (b) échelle assistée (1–5, ancres rédigées avant le test) : hiérarchie perçue, calme, conformité au langage d'interaction.
