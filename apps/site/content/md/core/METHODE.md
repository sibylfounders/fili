...
- **`npm run tokens:build`** — la **fidélité des valeurs et le contraste**. Enchaîne trois étapes : la génération (`tokens.css`, thème Tailwind, variables Figma), la **garde de fidélité** (`verify-ds-md.mjs` — nom technique hérité, cf. `MIGRATION-FILI.md` — qui confronte chaque token du package React à la valeur d'autorité de `DESIGN.md` : une divergence non déclarée dans `ds-md.map.mjs` **casse le build**), puis les **paires de contraste** (`validate-contrast.mjs`, seuils 4.5:1 pour le texte, 3:1 pour une bordure identifiante, en clair et en sombre). Le contrat de valeurs se rafraîchit par `npm run sync:ds-md`, jamais à la main.
