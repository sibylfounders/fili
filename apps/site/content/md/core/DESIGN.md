...
colors:
  ...
  on-primary: "#FFFFFF"
  accent: "#0891B2" # recalibré (était #06B6D4) : focus ring à 3.68:1 sur fond blanc, ≥ 3:1 WCAG 1.4.11
  secondary: "#E0E7FF" # ajouté en 1.12.0 — emphase intermédiaire de la famille primary : fond doux pour actions et insignes secondaires. Jamais un état sémantique (guardrail COLOR), et ne délimite jamais seul (1.23:1 sur blanc) : toujours porté par un contenu on-secondary
  secondary-hover: "#C7D2FE" # état hover du fond secondary — un cran plus soutenu, même teinte
  on-secondary: "#3730A3" # texte/icône sur secondary : 8.06:1 sur secondary, 6.66:1 sur secondary-hover — ≥ 4.5:1 partout où il porte du texte
  ...
  info: "#1D4ED8" # ajouté en 1.4.0 — tone info de l'alert : 6.70:1 sur blanc, 5.49:1 sur info-subtle. Volontairement distinct d'accent (guardrail : jamais la palette de marque pour un état sémantique)
  info-subtle: "#DBEAFE" # ajouté en 1.4.0 — fond de l'alert info
...
## Couleurs
Palette inspirée d'un registre indigo/cyan — technique et sobre, cohérent avec le positionnement du projet (documentation, précision, IA). `primary` porte les actions principales, `accent` sert de focus ring et de touches d'interface secondaires. Les rôles sémantiques (`danger`, `success`, `warning`) sont volontairement distincts des couleurs de marque (`primary`/`accent`) pour ne jamais confondre "action de marque" et "état sémantique" — c'est le même principe que la séparation style/tone déjà posée pour le bouton.
...
- **Hover d'un fond subtil** (style `lighter`) — un cran plus soutenu, le texte garde son token de tone ≥ 4.5:1 : `secondary-hover` (primary), `surface-hover` (neutral), `danger-subtle-hover`, `warning-subtle-hover`.
...
Le raisonnement complet vit dans `atelier/foundations/typography/TYPOGRAPHY-UX.md` ; ce fichier ne porte que les valeurs :
...
Le raisonnement du mouvement vit dans `atelier/languages/motion/` ; les autres vocabulaires tokenisés vivent dans `atelier/foundations/`. Ce fichier ne porte que les valeurs.
...
## Guardrails
- Ne jamais utiliser `primary` ou `accent` pour un état sémantique (erreur, succès, alerte) — ces rôles ont leurs propres tokens, ne pas les improviser à partir de la palette de marque.
