...
colors:
  ...
  on-primary: "#FFFFFF"
  accent: "#C026D3" # 1.33.0 (était #0891B2 cyan) : fuchsia-600 — focus ring à 4.71:1 sur blanc (≥ 3:1 WCAG 1.4.11), à 46° OKLCh de primary : visible même posé sur un fond primary
...
## Couleurs
Palette de marque à deux teintes depuis 1.33.0 — indigo (`primary`) et teal (`secondary`), accent fuchsia pour le focus. `primary` porte les actions principales, `secondary` la seconde voix de marque, `accent` sert exclusivement de focus ring. Les rôles sémantiques (`danger`, `success`, `warning`) sont volontairement distincts des couleurs de marque (`primary`/`accent`) pour ne jamais confondre "action de marque" et "état sémantique" — c'est le même principe que la séparation style/tone déjà posée pour le bouton.
...
Deux rôles sémantiques pouvant cohabiter à l'écran sont séparés d'**au moins 30° de teinte OKLCh**, sauf s'ils se distinguent nettement sur un autre axe (Δchroma ≥ 0,08 ou Δclarté ≥ 0,15) — la dérogation est alors documentée ici. C'est cette règle qui a écarté `info` de `primary` (Δ12,6° avant 1.33.0 : indiscernables en périphérie ou en légende) et choisi teal/fuchsia pour `secondary`/`accent`. Positions actuelles (clair) : danger 27° · warning 46° · success 150° · secondary 186° · info 249° · primary 277° · accent 323°.
