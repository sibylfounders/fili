...
RÈGLE [BORDER-R06] : le focus ring est **unifié pour tout le système** : couleur `accent` (≥ 3:1 sur les fonds d'usage, recalibrée pour ça en 1.3.0), largeur et écart tokenisés (`border.focus-width`, `border.focus-offset` — créés par cette fondation : trois composants déclaraient la couleur du ring et laissaient largeur/écart en déduction silencieuse, le travers exact que le système combat depuis tone.destructive_text).
STATUT : parti pris d'identité
SOURCE : S10, S3, S1
ÉNONCÉ : Notre anneau de focus est le même partout : une couleur, une largeur, un écart, définis une seule fois.
