...
RÈGLE : le doigt n'est pas une souris. Il est **imprécis** (une empreinte de contact d'environ 9 mm,
...
RÈGLE : **la cible n'est pas l'icône.** La zone qui reçoit le doigt (le *hit target*) peut être bien
...
RÈGLE : trois valeurs, et une lecture simple — **confort par défaut, plancher en dernier recours,
espacement toujours**.
...
RÈGLE : `touch.target-comfortable` est la valeur qu'on vise sans y penser. On ne descend à
...
RÈGLE : sous `touch.target-min`, le build **s'arrête et remonte**. Une cible plus petite que le
...
RÈGLE : une cible peut descendre sous `touch.target-min` dans **deux cas seulement**, tous deux
...
RÈGLE : ces deux exceptions se **déclarent**, elles ne se présument pas. Toute autre cible sous le
...
RÈGLE : les actions **primaires et fréquentes** d'un parcours au doigt visent la **zone
...
RÈGLE : on ne place jamais une cible tapable là où elle entre en **conflit avec un geste système**
...
RÈGLE : au doigt, il n'y a **pas de hover** — le **press** (l'appui) est le signal d'affordance. La
...
RÈGLE : l'action se déclenche au **relâchement sur la cible**, jamais au premier contact. Glisser le
...
RÈGLE : le **retour haptique** (vibration) est un supplément facultatif, jamais un canal unique : il
est absent de beaucoup d'appareils, désactivable, et invisible pour qui ne le sent pas.

## Robustesse et accessibilité

RÈGLE : la cible reste tapable **au zoom** (200 %, loupe) — les tailles dérivent de tokens, jamais
d'un px absolu qui ne suit pas l'agrandissement.

RÈGLE : la marge de `touch.target-comfortable` et de `touch.target-spacing` **est** l'accessibilité
...
RÈGLE : une cible n'existe **jamais que pour le doigt**. Toute cible tactile est aussi atteignable au
...
RÈGLE : toute nouvelle famille de cibles passe ces quatre questions :
...
|---|---|---|
| Cible minimale de 24 × 24 px CSS (avec exceptions inline / essentiel / espacement) | [WCAG 2.2 — 2.5.8 Target Size (Minimum)](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum) | Établi, standard d'accessibilité (niveau AA) |
| Cible confortable de 44 × 44 px CSS | [WCAG 2.1 — 2.5.5 Target Size (Enhanced)](https://www.w3.org/WAI/WCAG21/Understanding/target-size-enhanced) (AAA) ; [Apple — Human Interface Guidelines, Touchscreen gestures](https://developer.apple.com/design/human-interface-guidelines/gestures) (44 pt) | Établi — convergence WCAG AAA + Apple |
| Cible tactile ~48 dp et espacement ~8 dp | [Material Design — Accessibility, touch targets](https://m2.material.io/design/usability/accessibility.html#layout-and-typography) | Établi chez Material ; le système retient 44/8 (convergence WCAG/HIG) |
| Annulation au relâchement (down-event ≠ déclenchement) | [WCAG 2.2 — 2.5.2 Pointer Cancellation](https://www.w3.org/WAI/WCAG22/Understanding/pointer-cancellation.html) | Établi, standard d'accessibilité (niveau A) |
| Cible plus rapide si grande et proche (taille ↔ atteinte) | [Loi de Fitts](https://lawsofux.com/fittss-law/) (cf. `LAWS-UX`) | Référence établie en design |
| Trois crans (24 / 44 / 8) et « confort par défaut » | Décision d'identité interne, 2026-07-25 | Arbitrage interne, aligné sur les seuils sourcés |
