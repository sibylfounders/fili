...
| CardGroup | public | collection | cols 1-4/auto* ; density spacious/comfortable*/compact ; orientation ; mode ; separated ; outlined ; solo ; proximity | COLLECTION 1.0.1/1.0.0 | oui | css : `--border-focus-width/offset` inexistants (fallback 2px masquait le mauvais nom — corrigé) ; ~20 px hors échelle --space-* ; radius 6px hors échelle ; font-size en dur |
